...
public record PublicArticleSummaryResponse(
    ...
    public static PublicArticleSummaryResponse from(Article article) {
        return new PublicArticleSummaryResponse(article.getSlug(), article.getTitle(), article.getPretitle(),
                article.getExcerpt(), article.getPublishedAt(), article.getCoverImageUrl(), article.getCoverImageAlt());
    }
