...
describe('AdminArticleEditPage', () => {
  ...
  let getArticleResponse: Observable<AdminArticleDetail>;

  let updateArticleResponse: Observable<AdminArticleDetail>;

  let publishArticleResponse: Observable<AdminArticleDetail>;

  let withdrawArticleResponse: Observable<AdminArticleDetail>;
  ...
  let receivedUpdateRequest: UpdateArticleRequest | undefined;
  ...
  it('should load and render the article', () => {
    ...
    const titleInput = compiled.querySelector<HTMLInputElement>('#article-title');

    const pretitleInput = compiled.querySelector<HTMLInputElement>('#article-pretitle');

    const excerptTextarea = compiled.querySelector<HTMLTextAreaElement>('#article-excerpt');

    const bodyTextarea = compiled.querySelector<HTMLTextAreaElement>('#article-body');

    expect(titleInput?.value).toBe(originalArticle.title);

    expect(pretitleInput?.value).toBe(originalArticle.pretitle);

    expect(excerptTextarea?.value).toBe(originalArticle.excerpt);

    expect(bodyTextarea?.value).toBe(originalArticle.body);

    expect(compiled.textContent).toContain('Borrador');
  });
  ...
  it('should update the article when the form is submitted', () => {
    ...
    bodyTextarea.value = ' Cuerpo actualizado. ';

    bodyTextarea.dispatchEvent(new Event('input'));

    fixture.detectChanges();

    const form = compiled.querySelector<HTMLFormElement>('.article-form');
    ...
    expect(compiled.textContent).toContain('Cambios guardados correctamente');
  });
  ...
  it('should publish a complete saved article', () => {
    ...
    const publishButton = compiled.querySelector<HTMLButtonElement>('.editorial-action--publish');
    ...
    expect(compiled.textContent).toContain('Artículo publicado correctamente');

    expect(compiled.querySelector('.article-status')?.textContent).toContain('Publicado');

    expect(compiled.querySelector('.editorial-action--view')).toBeTruthy();

    expect(compiled.querySelector('.editorial-action--withdraw')).toBeTruthy();
  });
  ...
  it('should prevent publication when there are unsaved changes', () => {
    ...
    titleInput.value = 'Título modificado sin guardar';
    ...
    expect(publishedArticleId).toBeUndefined();

    expect(compiled.textContent).toContain('Guarda los cambios antes de modificar');
  });
  ...
  it('should withdraw a published article', () => {
    ...
    const withdrawButton = compiled.querySelector<HTMLButtonElement>('.editorial-action--withdraw');
    ...
    expect(compiled.textContent).toContain('Artículo retirado correctamente');

    expect(compiled.querySelector('.article-status')?.textContent).toContain('Retirado');

    const publishButton = compiled.querySelector<HTMLButtonElement>('.editorial-action--publish');

    expect(publishButton?.textContent).toContain('Volver a publicar');
  });
  ...
  it('should show a not found state when the article does not exist', () => {
    ...
    expect(compiled.textContent).toContain('Este artículo no existe');

    expect(compiled.querySelector('.article-form')).toBeNull();
  });
});
